...
contract FixedProductMarketMaker is ERC20, ERC1155TokenReceiver {
    ...
    function calcAddFundingAmounts(uint addedFunds) external view returns(uint[] memory)
    { 
        require(addedFunds > 0, "funding must be non-zero");

        // indexes 0 to length-2: shares returned for each position
        // index length-1: FPMM shares
        uint[] memory amounts = new uint[](positionIds.length + 1);
        ...
        if(poolShareSupply > 0) {
            ...
            for(uint i = 0; i < poolBalances.length; i++) {
                ...
                amounts[i] = addedFunds.sub(remaining);
            }
        ...
        amounts[amounts.length -1] = mintAmount;
        return amounts;
    }

    function calcRemoveFundingAmounts(uint sharesToBurn) external view returns(uint[] memory)
    {
        uint[] memory poolBalances = getPoolBalances();
